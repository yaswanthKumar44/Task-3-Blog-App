...
if ($query) {
    $search_term = '%' . $query . '%';
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM posts WHERE title LIKE :query OR content LIKE :query");
    $stmt->bindValue(':query', $search_term);
    $stmt->execute();
    $total_posts = $stmt->fetchColumn();
    $total_pages = ceil($total_posts / POSTS_PER_PAGE);

    $stmt = $pdo->prepare("SELECT p.*, u.username FROM posts p JOIN users u ON p.user_id = u.id WHERE p.title LIKE :query OR p.content LIKE :query ORDER BY p.created_at DESC LIMIT :limit OFFSET :offset");
    $stmt->bindValue(':query', $search_term);
